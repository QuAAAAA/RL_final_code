...
class VAAlphaMLP(nn.Module):
    """VA (2D) → 8-dim bias b = λ·tanh(MLP(v)), added to emotion one-hot."""
    ...
    def forward(self, va_01: torch.Tensor) -> torch.Tensor:
        """va_01: [2] → b: [8]  (before λ scaling)"""
        return self.net(va_01.float())
...
class IndexTTSPolicy:
    def __init__(
        ...
        self.va_alpha_mlp = VAAlphaMLP().to(device)
    ...
    def prepare_conditioning(
        ...
        with torch.no_grad():
            ...
            if va_01 is not None and emotion_vector is not None and any(v > 0 for v in emotion_vector):
                va_t = torch.tensor(list(va_01), device=device)
                y = torch.tensor(emotion_vector, device=device)          # one-hot [8]
                b = _VA_LAMBDA * self.va_alpha_mlp(va_t)                 # [8]
                weight_vector = (y + b).clamp(0.0, 1.4)                 # ŷ ∈ [0, 1.4]
    ...
    def rollout(
        ...
        for k in range(group_size):
            ...
            logprobs_list.append(torch.empty(end, device=device))
    ...
    def snapshot_reference(self, device: str | None = None) -> None:
        """Freeze a deep copy of gpt as the reference model for KL regularization."""
        import copy
        self._ref_gpt = copy.deepcopy(self.gpt)
        self._ref_device = device or self.device
        self._ref_gpt.to(self._ref_device)
        self._ref_gpt.eval()
    ...
    def compute_ref_logprobs(self, rollout: RolloutResult, k: int) -> torch.Tensor:
        ...
        gpt = self._ref_gpt
        device = self._ref_device

        codes = rollout.codes[k].long().unsqueeze(0).to(device)
        speech_cond_latent = rollout.speech_conditioning_latent.to(device)
        emovec = rollout.emovec.to(device)
        text_tokens = rollout.text_tokens.long().to(device)
        use_speed = rollout.use_speed.to(device)
        T = codes.shape[1]
    ...
    def compute_logprobs(self, rollout: RolloutResult, k: int) -> torch.Tensor:
        ...
        # Recompute ŷ = y + λ·tanh(MLP(v)) with gradient so MLP is trained end-to-end
        if (
            ...
            y = torch.tensor(rollout.base_emotion_vector, device=device)  # one-hot [8], no grad
            b = _VA_LAMBDA * self.va_alpha_mlp(va_t)                      # [8], has grad
            weight_vector = (y + b).clamp(0.0, 1.4)                       # ŷ ∈ [0, 1.4], has grad
